...
import warnings
from .weathercsvparser import get_data
...
def load_csvdata(time_steps, seperate=False):
    input = get_data('input.csv')
    output = get_data('output.csv')
    if not isinstance(input, pd.DataFrame):
        input = pd.DataFrame(input)
    if not isinstance(output, pd.DataFrame):
        output = pd.DataFrame(output)

    train_x, val_x, test_x = prepare_data(input, time_steps)
    train_y, val_y, test_y = prepare_data(output, time_steps, labels=False)
